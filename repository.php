...
function lireTransaction(array $transactions, array $wallets): void {
    if(empty($transactions)){
        affichage("Aucune transaction enregistrées");
        return;
    }
    ...
    foreach ($transactions as $index => $transaction) {
        ...
        $type = $montant >= 0 ? "Dépôt" : "Retrait";
        affichage("Transaction {$index} - {$type} de " . abs($montant) . "FCFA pour {$client}");
    }
};
